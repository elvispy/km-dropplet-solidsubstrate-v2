...
function solve_motion_discretized()
...
%     end
    ...
    probable_next_conditions = cell(1, 5);
    current_conditions = ProblemConditions_v2( ...
        ...
        dt, ...
        initial_height, ...
        initial_velocity, contact_points * spatial_step); % Last argument is contact radius
...
%     mechanical_energy_out = NaN; % TODO: Lab COef of restitution?
    ...
    while ( current_time < final_time) 
        ...
        [probable_next_conditions{3}, errortan(3)] = ...
            get_next_step_discretized(previous_conditions, contact_points, dt, PROBLEM_CONSTANTS);
        ...
        if abs(errortan(3)) < tan_tol * 1e-2 % If almost no error, we accept the solution
            ...
            [probable_next_conditions{4}, errortan(4)] = get_next_step_discretized(previous_conditions, contact_points + 1, dt, PROBLEM_CONSTANTS);
            % Lets try with one point less
            [probable_next_conditions{2}, errortan(2)] = get_next_step_discretized(previous_conditions, contact_points - 1, dt, PROBLEM_CONSTANTS);
            ...
            if (abs(errortan(3)) > abs(errortan(4)) ||  (abs(errortan(3)) > abs(errortan(2))))
                if abs(errortan(4)) <= abs(errortan(2))
                    ...
                    [~, errortan(5)] = get_next_step_discretized(previous_conditions, contact_points + 2, dt, PROBLEM_CONSTANTS);
                    ...
                    [~, errortan(1)] = get_next_step_discretized(previous_conditions, contact_points - 2, dt, PROBLEM_CONSTANTS);
